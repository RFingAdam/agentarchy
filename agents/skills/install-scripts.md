...
- `bin/oal-apply-system` runs root-owned system setup during ISO finalization.
- `bin/oal-apply-hardware` runs idempotent hardware-specific setup and is called by `oal-apply-system`.
- `bin/oal-finalize-user` runs the per-user runtime finalization (skill symlinks, xdg-user-dirs, mime defaults, `install/user/all.sh`). Shipped user defaults are seeded by `/etc/skel` from `oal-settings`, not by this command. `bin/oal-reinstall-configs` is the explicit destructive resync of those defaults into an existing user's `$HOME`.
- leaf scripts under `install/` are sourced by `run_logged $OAL_INSTALL/path/to/script.sh` and intentionally do not have shebangs.
- avoid `exit` in sourced setup scripts unless intentionally aborting setup.
- use `$OAL_INSTALL` and `$OAL_PATH` instead of hard-coded Agentarchy paths.
