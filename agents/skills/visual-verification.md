...
Read this before finishing any change with a visual effect: Agentarchy shell
...
```bash
oal capture screenshot fullscreen save
```

The command prints the saved path and writes to the configured Pictures
directory. Use `oal screenshot` for the interactive smart-region flow.
...
```bash
oal screenrecord --fullscreen
# Exercise the changed behavior.
oal screenrecord --stop-recording
```
