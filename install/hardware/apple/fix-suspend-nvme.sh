...
if [[ $MACBOOK_MODEL =~ MacBook(8,1|9,1|10,1)|MacBookPro13,[123]|MacBookPro14,[123] ]]; then
  ...
  if [[ -f $NVME_DEVICE ]]; then
    ...
    sudo tee /etc/systemd/system/oal-nvme-suspend-fix.service >/dev/null <<'EOF'
[Unit]
Description=Agentarchy NVMe Suspend Fix for MacBook
...
EOF
    ...
    sudo systemctl enable oal-nvme-suspend-fix.service
  else
