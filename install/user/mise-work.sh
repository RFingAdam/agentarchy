...
if [[ ${OAL_SKIP_MISE:-0} == 1 ]]; then
  echo "skipping mise-work.sh: OAL_SKIP_MISE=1"
  exit 0
fi
if ! command -v mise >/dev/null; then
  echo "skipping mise-work.sh: mise is not installed"
  exit 0
fi
...
# oal-provision-owner finalizes the user at first boot.
case ${OAL_SETUP_CONTEXT:-runtime} in
  iso-chroot) NODE_PACKAGE_DIR=/opt/packages ;;
  provision-owner) NODE_PACKAGE_DIR=/var/lib/oal/provisioning/packages ;;
  *) NODE_PACKAGE_DIR="" ;;
esac
...
if [[ -n $NODE_PACKAGE_DIR ]]; then
  ...
  if [[ -z $NODE_TARBALL ]]; then
    if [[ ${OAL_SETUP_CONTEXT:-} == "provision-owner" ]]; then
