SNAPPER_CONFIG_PATH="${OAL_SNAPPER_CONFIG_PATH:-/etc/snapper/configs/root}"
SNAPPER_CONF_PATH="${OAL_SNAPPER_CONF_PATH:-/etc/conf.d/snapper}"
template="${OAL_SNAPPER_TEMPLATE:-${OAL_PATH:-/usr/share/agentarchy}/default/snapper/root}"

echo "Configuring Agentarchy Snapper snapshot retention"

if [[ ! -f $SNAPPER_CONFIG_PATH ]]; then
  mkdir -p "$(dirname "$SNAPPER_CONFIG_PATH")"

  if [[ ${OAL_SNAPPER_CONFIGURE_TEST:-0} == "1" ]]; then
    : >"$SNAPPER_CONFIG_PATH"
  else
