# The setup form: every question Agentarchy asks a human to describe their machine
...
# (oal-provision-owner). Sourced by both, so the copies cannot drift the way
...
OAL_KEYBOARD_LAYOUTS=$'English (US)|us
English (UK)|uk
...
OAL_USERNAME_PATTERN='^[a-z_][a-z0-9_-]*[$]?$'
OAL_RESERVED_USERNAMES='^(root|bin|daemon|mail|ftp|http|nobody|dbus|systemd-coredump|systemd-network|systemd-oom|systemd-journal-remote|systemd-resolve|systemd-timesync|tss|uuidd|alpm|git|avahi|cups|lp|_talkd|polkitd|rtkit|qemu|brltty|gluster|rpc|libvirt-qemu|pcscd|nvidia-persistenced|sddm)$'
OAL_HOSTNAME_PATTERN='^[A-Za-z0-9]([A-Za-z0-9-]{0,61}[A-Za-z0-9])?$'
OAL_HOSTNAME_DEFAULT='oal'
...
oal_prompt_username() {
  ...
  while true; do
    ...
    if [[ "$username" =~ $OAL_USERNAME_PATTERN ]]; then
      if [[ "$username" =~ $OAL_RESERVED_USERNAMES ]]; then
        notice "Username is reserved for system" 1
      elif oal_username_taken "$username"; then
...
oal_prompt_hostname() {
  local status
  while true; do
    hostname=$(gum input --placeholder "Letters, digits, and dashes (or return for 'oal')" --prompt.foreground="#845DF9" --prompt "Hostname> ") && status=0 || status=$?
    ((status == 0)) || return $status

    if [[ -z $hostname ]]; then
      hostname="$OAL_HOSTNAME_DEFAULT"
      return 0
    elif [[ "$hostname" =~ $OAL_HOSTNAME_PATTERN ]]; then
      return 0
    else
