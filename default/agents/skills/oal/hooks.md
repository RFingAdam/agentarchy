...
Hooks live in `~/.config/oal/hooks/<name>.d/` — one directory per event,
holding any number of independent scripts. Install with
`oal hook install <name> <script>` (copies the script in and makes it
executable). The runner also executes a flat `~/.config/oal/hooks/<name>`
...
├── post-update.d/          # During `oal update`, after system packages and migrations
├── pre-refresh-pacman.d/   # Before `oal refresh pacman` re-syncs packages
