...
Agentarchy configures Hyprland in Lua. User files are loaded after Agentarchy's
defaults, so overrides go here:

```
~/.config/hypr/
├── hyprland.lua       # Main config (loads Agentarchy defaults, then user files)
...
- Use `oal refresh hyprland` to reset the Lua config files to defaults

The two `.conf` files are read by separate processes, so `hyprctl` neither
applies nor validates them:
- `hyprsunset.conf` (night light): apply changes with `oal restart hyprsunset`; reset with `oal refresh hyprsunset`
...
View current bindings: `oal menu keybindings --print`

**IMPORTANT: When re-binding an existing key:**

1. First check existing bindings: `oal menu keybindings --print`
...
Window rules go in `~/.config/hypr/hyprland.lua` or a required Lua module. Prefer Agentarchy's `o.window(match, rules)` helper — see examples in `$OAL_PATH/default/hypr/windows.lua`.
