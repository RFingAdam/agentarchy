...
class TranscodeAction(GObject.GObject, Nautilus.MenuProvider):
    def _launch_transcode(self, paths):
        wrapper = shutil.which("oal-launch-floating-terminal-with-presentation")
        binary = shutil.which("oal-transcode")
    ...
    def _make_item(self, paths):
        ...
        item = Nautilus.MenuItem(
            name="AgentarchyTranscodeNautilus::transcode",
            label=label,
    ...
    def _tools_available(self):
        return bool(
            shutil.which("oal-launch-floating-terminal-with-presentation")
            and shutil.which("oal-transcode")
        )
